...
Then /^I should be able to bring up interface ([^ ]*)$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to bring up interface #{interface}"
...
Then /^I should not be able to bring up interface ([^ ]*)$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not be able to bring up interface #{interface}"
  local, remote, command = SUT.test_and_drop_results \
    "root", "wic.sh ifup #{interface}"
  local.should == 0; remote.should == 0
...
Then /^all server daemons should be up and running$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then all server daemons should be up and running"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ps aux | grep wicked"
  local.should == 0; remote.should == 0
...
Then /^no more server daemon should be left$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then no more server daemon should be left"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ps aux | grep wicked"
  local.should == 0; remote.should == 0
...
Then /^the interface ([^ ]*) should be up and running$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then the interface #{interface} should be up and running"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip link show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "state UP"
end

Then /^the interface ([^ ]*) should be down$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then the interface #{interface} should be down"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip link show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^the brief status of ([^ ]*) should be "([^"]*)"$/ do |interface, state|
  SUT.test_and_drop_results "root", "log.sh Step: Then the brief status of #{interface} should be #{state}"
...
Then /^I should be able to ping (?:a |the )reference machine from my static address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping a reference machine from my static address"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{STAT4_REF0}"
  local.should == 0; remote.should == 0
  command.should == 0
  #
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping6 -q -c1 -W1 #{STAT6_REF0}"
  local.should == 0; remote.should == 0
  command.should == 0
end

Then /^I should not be able to ping (?:a |the )reference machine from my static address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not be able to ping a reference machine from my static address"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{STAT4_REF0}"
  local.should == 0; remote.should == 0
  command.should_not == 0
...
Then /^I should be able to ping (?:a |the )reference machine from my dynamic address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping a reference machine from my dynamic address"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{DHCP4_REF0}"
  local.should == 0; remote.should == 0
  command.should == 0
  #
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping6 -q -c1 -W1 #{RADVD_REF0}"
  local.should == 0; remote.should == 0
  command.should == 0
  #
...
Then /^I should not be able to ping (?:a |the )reference machine from my dynamic address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not be able to ping a reference machine from my dynamic address"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{DHCP4_REF0}"
  local.should == 0; remote.should == 0
  command.should_not == 0
...
Then /^I should be able to ping a host in the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping a host in the outside World"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{STAT4_OUT}"
  local.should == 0; remote.should == 0
  command.should == 0
  #
  # Ugly hack to make sure ping6 works from reference to the outside
  loop do
    local, remote, command = REF.test_and_drop_results "testuser", "ping6 -q -c1 -W1 #{STAT6_OUT}"
    break if command == 0
  end
  #
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping6 -q -c1 -W1 #{STAT6_OUT}"
  local.should == 0; remote.should == 0
  command.should == 0
end

Then /^I should not be able to ping a host in the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not be able to ping a host in the outside World"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "ping -q -c1 -W1 #{STAT4_OUT}"
  local.should == 0; remote.should == 0
  command.should_not == 0
...
Then /^([^ ]*) should use the static addresses$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should use the static addresses"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
  case interface
    when "eth0"
...
Then /^([^ ]*) should not use the static addresses$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should not use the static addresses"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
  case interface
...
Then /^([^ ]*) should use the dynamic addresses$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should use the dynamic addresses"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^([^ ]*) should not use the dynamic addresses$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should not use the dynamic addresses"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^([^ ]*) should not use any address$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should not use any address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface} | grep -v fe80:"
  local.should == 0; remote.should == 0
  out.should_not include "inet"
end

Then /^I should have the correct MTU on eth0$/  do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should have the correct MTU on eth0"
  out, local, remote, command = SUT.test_and_store_results_together \
...
Then /^I should have static routes to the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should have static routes to the outside World"
  out, local, remote, command = SUT.test_and_store_results_together \
...
Then /^I should not have static routes to the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not have static routes to the outside World"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -4 route show"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should have dynamic routes to the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should have dynamic routes to the outside World"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -4 route show"
  local.should == 0; remote.should == 0; command.should == 0
  ...
  out, * = REF.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0 | grep \"scope link\""
...
Then /^I should not have dynamic routes to the outside World$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not have dynamic routes to the outside World"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -4 route show"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^eth0 should have a dynamic address after a while$/ do
  ...
  SUT.test_and_drop_results "root", "log.sh Step: eth0 should have a dynamic address after a while"
  local, remote, command = SUT.test_and_drop_results \
    "testuser", "wait_for_cmd_success.sh \"ip address show dev eth0 | grep #{DHCP4_SUT0}\""
  local.should == 0; remote.should == 0; command.should == 0
end

Then /^I should follow the correct IPv4 route to a reference machine from eth1$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should follow the correct IPv4 route to a reference machine from eth1"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "tracepath -n #{STAT4_GAT}"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "#{STAT4_REF1}"
end

Then /^I should follow the correct IPv6 route to a reference machine from eth1$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should follow the correct IPv6 route to a reference machine from eth1"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "tracepath6 -l 64 -n #{STAT6_GAT}"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "#{STAT6_REF1}"
end

Then /^both machines should have a new ([^ ]*) card$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then both machines should have a new #{interface} card"
  out, local, remote, command = REF.test_and_store_results_together \
    "testuser", "ip address show"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^([^ ]*) should be enslaved$/ do |interface|
  ...
  SUT.test_and_drop_results "root", "log.sh Step: Then #{interface} should be enslaved"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev #{interface}"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should be able to ping the other side of the aggregated link$/ do
  ...
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping the other side of the aggregated link"
  local, remote, command = SUT.test_and_drop_results \
...
Then /^there should be a new ([^ ]*) card$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then there should be a new #{interface} card"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include ": #{interface}"
...
Then /^there should not be the ([^ ]*) card anymore$/ do |interface|
  SUT.test_and_drop_results "root", "log.sh Step: Then there should not be the #{interface} card anymore"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^the ([^ ]*) card should still be there$/ do |interface|
  ...
  SUT.test_and_drop_results "root", "log.sh Step: Then the #{interface} card should still be there"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include ": #{interface}"
...
Then /^the bridge should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then the bridge should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br1"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^the macvtap interface should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then the macvtap interface should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev macvtap1"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should receive the answer of the ARP ping on \/dev\/tapX$/ do
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /tmp/tests/macvtap_results.txt"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should be able to ping the other side of the infiniband link$/ do
  ...
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping the other side of the infiniband link"
  local, remote, command = SUT.test_and_drop_results \
...
Then /^I should be able to ping the other side of the infiniband child link$/ do
  ...
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping the other side of the infiniband child link"
  local, remote, command = SUT.test_and_drop_results \
...
Then /^br0\.1 should have the correct address$/ do
  ...
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br0.1"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^br0 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then br0 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{VLAN4_SUT0}"
  out.should include "inet6 #{VLAN6_SUT0}"
end
...
Then /^bond1 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then bond1 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev bond1"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should be able to ping bond(\d) on the other side$/ do |x|
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping bond#{x} on the other side"
...
Then /^br1 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then br1 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br1"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{BOND4_SUT}"
...
Then /^br1.42 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then br1.42 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br1.42"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should be able to ping bond0.42 on the other side$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should be able to ping bond0.42 on the other side"
...
Then /^bond0.42 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then bond0.42 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev bond0.42"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{V42_4_SUT}"
  out.should include "inet6 #{V42_6_SUT}"
end
...
Then /^bond0.73 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then bond0.73 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev bond0.73"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^br42 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then br42 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br42"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{V42_4_SUT}"
  out.should include "inet6 #{V42_6_SUT}"
end
...
Then /^br73 should have the correct address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then br73 should have the correct address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev br73"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{V73_4_SUT}"
  out.should include "inet6 #{V73_6_SUT}"
end
...
Then /^I should have the statically declared DNS server$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should have the statically declared DNS server"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "nameserver #{STAT4_OUT}"
end

Then /^I should obtain a DNS server by DHCPv4$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain a DNS server by DHCPv4"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should not obtain a DNS server by DHCPv4$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain a DNS server by DHCPv4"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^the capture file should contain a DHCP release$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then the capture file should contain a DHCP release"
...
Then /^I should obtain an autonomous IPv6 address$/ do
  ...
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet6 #{RADVD_SUT0}"
end

Then /^I should not obtain an autonomous IPv6 address$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain an autonomous IPv6 address"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should_not include "inet6 #{RADVD_SUT0}"
...
Then /^I should obtain an address by DHCPv4$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain an address by DHCPv4"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet #{DHCP4_SUT0}"
...
Then /^I should not obtain an address by DHCPv4$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain an address by DHCPv4"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should_not include "inet #{DHCP4_SUT0}"
...
Then /^I should obtain an address by DHCPv6$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain an address by DHCPv6"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should include "inet6 #{DHCP6_SUT0}"
...
Then /^I should not obtain an address by DHCPv6$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain an address by DHCPv6"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should_not include "inet6 #{DHCP6_SUT0}"
...
Then /^no address should be tentative$/ do
  SUT.test_and_drop_results "root", "log.sh Step: No address should be tentative"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 address show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should obtain a default IPv6 route$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain a default IPv6 route"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 route show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
  out.should match "^#{RADVD_SUT0}"
end

Then /^I should not obtain a default IPv6 route$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain a default IPv6 route"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "ip -6 route show dev eth0"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should obtain a DNS server by RDNSS$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain a DNS server by RDNSS"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should not obtain a DNS server by RDNSS$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain a DNS server by RDNSS"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should obtain a DNS server by DHCPv6$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should obtain a DNS server by DHCPv6"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^I should not obtain a DNS server by DHCPv6$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then I should not obtain a DNS server by DHCPv6"
  out, local, remote, command = SUT.test_and_store_results_together \
    "testuser", "cat /etc/resolv.conf"
  local.should == 0; remote.should == 0; command.should == 0
...
Then /^the setup should be finished$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then the setup should be finished"
  @setup_in_progress.should be false
end

Then /^the setup should still be in progress$/ do
  SUT.test_and_drop_results "root", "log.sh Step: Then the setup should still be in progress"
