...
function _step_end
{
  ...
  if [ "$coredump" != "" ]; then
    if [ ! -h $logs/coredump ]; then
      dest=$(readlink -m $step | sed "s/^$logs//")

      cp /$coredump $step/$coredump
      ln -s $dest/$coredump $logs/core_dump

      binary=$(gdb -ex "quit" -c $coredump 2>&1 \
                 | grep "generated by" \
                 | sed -e 's/.*[`]//g; s/[ ].*//g')
      cp $binary $step/$binary
      ln -s $dest/$binary $logs/core_binary

      gdb -ex 'bt full' -ex 'quit' -c $coredump $binary \
                 > $step/backtrace.txt
      ln -s $dest/backtrace.txt $logs/core_backtrace
    fi
  fi
